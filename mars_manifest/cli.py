...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "pack":
        ...
        print(rpt.loss_markdown(
            PackingEngine(catalog, a).loss_tolerance(packing, unlock_rules())))
        if packing.warnings:
            print("\nWarnings:")
    ...
    if args.command == "manifests":
        ...
        # the flight-at-a-glance table: SUMMARY.md + the marked README section
        summary = rpt.flight_summary_markdown(
            result, a.get("fleet.payload_mass_per_ship_t"))
        spath = out_dir / "SUMMARY.md"
        spath.write_text(
            "# Flight summary\n\n*Generated by `mars manifests` — do not "
            "hand-edit; the freshness test enforces regeneration.*\n\n"
            + summary + "\n", encoding="utf-8")
        print(f"Wrote {jpath}")
        print(f"Wrote {cpath}")
        print(f"Wrote {spath}")
        readme = out_dir.parent.parent / "README.md"
        ...
        if readme.exists():
            ...
            if begin in text and end in text:
                ...
                text = (head + begin
                        + "\n<!-- generated by `mars manifests`; do not hand-edit -->\n"
                        + summary + "\n" + end + tail)
                readme.write_text(text, encoding="utf-8")
                print(f"Refreshed flight summary in {readme}")
        return 0
