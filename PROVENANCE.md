...
| `payload_volume_per_ship_m3` | 1000 | **C — flagged (was B)** | **Verification 2026-07-09 found drift:** the [Wikipedia Starship infobox](https://en.wikipedia.org/wiki/SpaceX_Starship) now lists **614 m³** payload volume for the current configuration; the ~1,000 m³ figure traces to earlier 9 m × 18 m fairing statements ([NSF specs thread](https://forum.nasaspaceflight.com/index.php?topic=50049.240)) and a stretched-fairing option. Impact check: at 614 m³/ship the program plan still fits every window (worst per-ship stow ~460 m³; precursor batch effective volume 61% of the smaller capacity vs 37% of the larger), so no conclusion changes — but volume margin is roughly half what the model displays. Kept at 1,000 for the §7 regression contract; flag any manifest whose per-ship volume approaches ~600 m³. |
...
| `solar_kg_per_m2` | 1.5 | **D — flagged** | **Most optimistic input in the file**, and verification weakened it further: the flown ROSA demonstrator is **325 kg** ([Wikipedia: ROSA](https://en.wikipedia.org/wiki/Roll_Out_Solar_Array), verified) over a ~6 × 13.7 m wing ([eoPortal](https://www.eoportal.org/satellite-missions/iss-rosa)) ≈ **~4 kg/m²** — neither source now states the 2.95 kg/m² previously cited. Our 1.5 is therefore ~2.7× lighter than flown hardware; vendor projections for large modular arrays support the direction ([Redwire flysheet](https://rdw.com/wp-content/uploads/2023/06/redwire-roll-out-solar-array-flysheet.pdf)) but not the number. At flown density the 11,805 m² array is ~47 t instead of 17.7 t — still small next to the 290 t dust-storm battery, so the solar-vs-fission conclusion survives regardless. |
...
| [Wikipedia — Starship](https://en.wikipedia.org/wiki/SpaceX_Starship) | ~1,000 m³ payload volume | ⚠️ **drift** — infobox now 614 m³; see §1 flag |
...
| [Wikipedia](https://en.wikipedia.org/wiki/Roll_Out_Solar_Array) + [eoPortal — ROSA](https://www.eoportal.org/satellite-missions/iss-rosa) | flown areal density ~2.95 kg/m² | ⚠️ **weakened** — sources support 325 kg / 6×13.7 m ≈ ~4 kg/m²; see §2 flag |
