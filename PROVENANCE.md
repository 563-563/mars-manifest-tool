...
| `payload_volume_per_ship_m3` | 614 | **B — re-anchored 2026-07-09** | The current published Starship payload volume ([Wikipedia infobox](https://en.wikipedia.org/wiki/SpaceX_Starship), verified). Replaces the ~1,000 m³ carried from earlier 9 m × 18 m fairing statements — the baseline now uses the verified number rather than an optimistic historical one (policy: anchor to real data, never keep a known-stale value and downgrade its tier). Consequence applied: program window 2035-05 is volume-bound at this bay size and flies **11 ships instead of 10**. §7 mass-side regression targets are unaffected (they never depended on bay volume). |
